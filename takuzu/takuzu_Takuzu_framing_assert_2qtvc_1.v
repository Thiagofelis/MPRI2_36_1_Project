...
Axiom framing_assert_1 :
  forall (g:array elem) (n:Numbers.BinNums.Z) (k:Numbers.BinNums.Z) (e1:elem)
    (e2:elem),
  (0%Z <= n)%Z /\ (n <= (length g))%Z ->
  ((0%Z <= n)%Z /\ (n <= (length g))%Z) /\
  (0%Z <= k)%Z /\ (k <= (length g))%Z -> forall (i:Numbers.BinNums.Z),
  (0%Z <= i)%Z /\ (i <= k)%Z ->
  ((mixfix_lbrb (mixfix_lblsmnrb (mixfix_lblsmnrb g n e1) n e2) i) =
   (mixfix_lbrb (mixfix_lblsmnrb g n e2) i)).
...
Axiom H1 : (n <= (length g))%Z.

Axiom Ensures :
  forall (i:Numbers.BinNums.Z), (0%Z <= i)%Z /\ (i <= (length g))%Z ->
  ((mixfix_lbrb (mixfix_lblsmnrb (mixfix_lblsmnrb g n e1) n e2) i) =
   (mixfix_lbrb (mixfix_lblsmnrb g n e2) i)).

(* Why3 goal *)
Theorem framing_assert_2'vc :
  ((mixfix_lblsmnrb (mixfix_lblsmnrb g n e1) n e2) =
   (mixfix_lblsmnrb g n e2)).
Proof.
